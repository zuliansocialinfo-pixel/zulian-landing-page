...
groups={
 ...
 'Zulian':[('Architettura Digitale','architettura-digitale'),('Chi sono','chi-sono'),('Certificazioni e formazione','certificazioni'),('Reggio Calabria','reggio-calabria'),('Contatti','contatti')],
...
def shell(title,description,body,route='/',noindex=False):
 ...
 header='''<a class="skip" href="#main">Salta al contenuto</a><header class="site-header"><div class="shell header-inner"><a class="brand" href="/" aria-label="Zulian Architettura Digitale, home"><img class="brand-original" src="/assets/zulian-logo-452.webp" srcset="/assets/zulian-logo-452.webp 452w, /assets/zulian-logo-904.webp 904w" sizes="(max-width: 700px) 194px, 226px" alt="Zulian Architettura Digitale" width="226" height="66"></a><nav class="desktop-nav" aria-label="Navigazione principale"><a href="/siti-web/">Siti web</a><a href="/modelli/">Modelli</a><a href="/prezzi/">Prezzi</a><a href="/automazioni-ai/">Automazioni e AI</a><details><summary>Visibilità</summary><div><a href="/seo-local-seo/">SEO e Local SEO</a><a href="/geo-ai-search/">GEO e AI Search</a></div></details><details><summary>Metodo</summary><div><a href="/metodo/">Metodo di lavoro</a><a href="/qualita/">Qualità</a><a href="/certificazioni/">Certificazioni e formazione</a><a href="/chi-sono/">Chi sono</a></div></details><a href="/guide/">Guide</a><a class="btn primary" href="/advisor/">Trova la soluzione</a></nav><button class="menu-button" data-menu-open aria-controls="menu" aria-expanded="false">Menu +</button></div></header>'''
...
for page in PAGES:
 ...
 if is_article:
  ...
  body+='<nav class="article-toc" aria-label="Indice della guida"><h2>In questa guida</h2><ol>'+''.join(f'<li><a href="#{e(sec.get("id", "section-"+str(i+1)))}">{e(sec["heading"])}</a></li>' for i,sec in enumerate(page.get('sections',[])))+'</ol></nav>'
 body+=page.get('before','')
 if route=='/prezzi/':body+='<section id="pacchetti" aria-label="Pacchetti sito a prezzo fisso">'+offers()+comparison(DATA)+'</section>'
 body+='<div class="prose">'
 for section_index,section in enumerate(page.get('sections',[]),1):
  section_id=section.get('id','section-'+str(section_index))
  body+=f'<section id="{e(section_id)}"><h2>{e(section["heading"])}</h2>'
  for paragraph in section.get('paragraphs',[]):body+=f'<p>{e(paragraph)}</p>'
  if section.get('items'):body+='<ul>'+''.join(f'<li>{e(item)}</li>' for item in section['items'])+'</ul>'
 ...
 if route=='/prezzi/':body+='<section aria-label="Domande frequenti sul listino"><h2>Dubbi frequenti, risposte precise.</h2>'+faq()+'</section>'
...
# Effective registry, including release pages; write_content.py regenerates the
# original editorial input on each clean build before this overlay is applied.
